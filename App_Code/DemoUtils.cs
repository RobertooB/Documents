...
public class DemoDataService : DataService {
    ...
    public static string GetDemoDataRootPath() {
        return CommonUtils.MapPath(DemoDataRootVirtPath);
    }
...
public class DemoAvatarService : AvatarService {
    ...
    public static string GetAvatarFolderRootPath() {
        return CommonUtils.MapPath(AvatarFolderVirtPath);
    }
}
...
public class DemoThumbnailService : ThumbnailService {
    ...
    public static string GetThumbnailFolderRootPath() {
        return CommonUtils.MapPath(ThumbnailFolderVirtPath);
    }
}
...
public class DemoAppRegistry {
    ...
    static readonly TimeSpan FirstClosedAppDisposingInterval = TimeSpan.FromSeconds(5);
    static readonly TimeSpan GeneralClosedAppDisposingInterval = TimeSpan.FromMinutes(5);
    ...
    void EnsureAppDisposingActivated() {
        if(!AppDisposingActivated) {
            PerformActionByTimeInterval(DisposeOldApps, FirstAppDisposingInterval, GeneralAppDisposingInterval);
            PerformActionByTimeInterval(DisposeAllClosedApps, FirstClosedAppDisposingInterval, GeneralClosedAppDisposingInterval);
            AppDisposingActivated = true;
        }
    }

    void PerformActionByTimeInterval(Action action, TimeSpan firstInterval, TimeSpan generalInterval) {
        CommonUtils.ExecuteActionWithDelay(() => OnDelayedPerformAction(action, generalInterval), firstInterval);
    }
    void OnDelayedPerformAction(Action action, TimeSpan nextInterval) {
        try {
            action();
        }
        finally {
            PerformActionByTimeInterval(action, nextInterval, nextInterval);
        }
    }
    ...
    void DisposeAllClosedApps() {
        DisposeClosedApps(DemoDataService.GetDemoDataRootPath());
        DisposeClosedApps(DemoAvatarService.GetAvatarFolderRootPath());
        DisposeClosedApps(DemoThumbnailService.GetThumbnailFolderRootPath());
    }
    void DisposeClosedApps(string path) {
        CommonUtils.ExecuteActionSafe(() => DisposeClosedAppsUnsafe(path));
    }
    void DisposeClosedAppsUnsafe(string path) {
        int demoKeyLength = SiteModeUtils.GenerateDemoKey().Length;
        DirectoryInfo dir = new DirectoryInfo(path);
        var subDirs = dir.GetDirectories()
            .Where(d => d.Name.Length == demoKeyLength && !AppDictionary.ContainsKey(d.Name));
        foreach(DirectoryInfo subDir in subDirs)
            CommonUtils.ExecuteActionSafe(() => subDir.Delete(true));
    }
}
...
public class SiteModeUtils {
    ...
    public static string GenerateDemoKey() {
        return Guid.NewGuid().ToString();
    }

    public static void SafeDeleteDirectory(string dirPath) {
        CommonUtils.ExecuteActionSafe(() => {
